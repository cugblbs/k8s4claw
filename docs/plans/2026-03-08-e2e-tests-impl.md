# E2E Lifecycle Tests Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add 7 envtest-based E2E lifecycle tests that verify full operator reconciliation across multiple sub-resources and controller interactions.

**Architecture:** Single file `internal/controller/e2e_lifecycle_test.go` in the existing `controller` package, reusing `suite_test.go`'s envtest environment. Each scenario is a top-level `Test*` function with ordered `t.Run` subtests. Each scenario uses a unique namespace for isolation.

**Tech Stack:** Go 1.25, controller-runtime envtest, standard `testing` package, existing `waitForCondition`/`createNamespace` helpers

---

## Task 1: OpenClaw Full Lifecycle

**Files:**
- Create: `internal/controller/e2e_lifecycle_test.go`
...
func TestE2E_OpenClawFullLifecycle(t *testing.T) {
	...
	t.Run("config update triggers StatefulSet update", func(t *testing.T) {
		// Get current STS generation.
		var stsBefore appsv1.StatefulSet
		if err := k8sClient.Get(ctx, nn, &stsBefore); err != nil {
			t.Fatalf("failed to get StatefulSet: %v", err)
		}
		genBefore := stsBefore.Generation
		...
		if latest.Spec.Config == nil {
			latest.Spec.Config = &clawv1alpha1.JSON{}
		}
		...
		// Verify StatefulSet generation incremented (template hash changed).
		waitForCondition(t, e2eTimeout, testInterval, func() (bool, error) {
			var stsAfter appsv1.StatefulSet
			if err := k8sClient.Get(ctx, nn, &stsAfter); err != nil {
				return false, err
			}
			return stsAfter.Generation > genBefore, nil
		})
	})

	t.Run("delete cleans up everything", func(t *testing.T) {
		// Re-fetch and delete.
		var latest clawv1alpha1.Claw
		if err := k8sClient.Get(ctx, nn, &latest); err != nil {
			t.Fatalf("failed to get Claw: %v", err)
		...
		if err := k8sClient.Get(ctx, nn, &appsv1.StatefulSet{}); client.IgnoreNotFound(err) != nil {
			t.Errorf("StatefulSet still exists or error: %v", err)
		} else if err == nil {
			t.Error("StatefulSet still exists after Claw deletion")
		}
	})
}
```
...
```bash
cd /home/willamhou/codes/k8s4claw && KUBEBUILDER_ASSETS="$(go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use 1.32.x -p path 2>/dev/null)" go test -race -run TestE2E_OpenClawFullLifecycle -v -timeout 120s ./internal/controller/
```
...
```bash
git add internal/controller/e2e_lifecycle_test.go
git commit -m "test: add E2E OpenClaw full lifecycle test"
```

---

## Task 2: Multi-Runtime Verification

**Files:**
- Modify: `internal/controller/e2e_lifecycle_test.go`

**Step 1: Add the test**

Append to `e2e_lifecycle_test.go`:
...
```bash
cd /home/willamhou/codes/k8s4claw && KUBEBUILDER_ASSETS="$(go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use 1.32.x -p path 2>/dev/null)" go test -race -run TestE2E_MultiRuntime -v -timeout 120s ./internal/controller/
```
...
```bash
git add internal/controller/e2e_lifecycle_test.go
git commit -m "test: add E2E multi-runtime verification test"
```

---

## Task 3: Channel + IPC Bus Injection
...
func TestE2E_ChannelWithIPCBus(t *testing.T) {
	...
	t.Run("delete claw decrements refCount", func(t *testing.T) {
		var latest clawv1alpha1.Claw
...
```bash
cd /home/willamhou/codes/k8s4claw && KUBEBUILDER_ASSETS="$(go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use 1.32.x -p path 2>/dev/null)" go test -race -run TestE2E_ChannelWithIPCBus -v -timeout 120s ./internal/controller/
```
...
```bash
git add internal/controller/e2e_lifecycle_test.go
git commit -m "test: add E2E channel + IPC Bus injection test"
```

---

## Task 4: Security and Network
...
Add import for `networkingv1 "k8s.io/api/networking/v1"` at the top, then append:
...
```bash
cd /home/willamhou/codes/k8s4claw && KUBEBUILDER_ASSETS="$(go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use 1.32.x -p path 2>/dev/null)" go test -race -run TestE2E_SecurityAndNetwork -v -timeout 120s ./internal/controller/
```
...
```bash
git add internal/controller/e2e_lifecycle_test.go
git commit -m "test: add E2E security and network test"
```

---

## Task 5: Persistence Reclaim Policies
...
Add import for `"k8s.io/apimachinery/pkg/api/resource"`, then append:
...
```bash
cd /home/willamhou/codes/k8s4claw && KUBEBUILDER_ASSETS="$(go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use 1.32.x -p path 2>/dev/null)" go test -race -run TestE2E_PersistenceReclaimPolicies -v -timeout 120s ./internal/controller/
```
...
```bash
git add internal/controller/e2e_lifecycle_test.go
git commit -m "test: add E2E persistence reclaim policy tests"
```

---

## Task 6: Webhook Validation
...
```bash
cd /home/willamhou/codes/k8s4claw && KUBEBUILDER_ASSETS="$(go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use 1.32.x -p path 2>/dev/null)" go test -race -run TestE2E_WebhookValidation -v -timeout 120s ./internal/controller/
```
...
```bash
git add internal/controller/e2e_lifecycle_test.go
git commit -m "test: add E2E webhook validation tests"
```

---

## Task 7: SelfConfig Lifecycle
...
Add import for `rbacv1 "k8s.io/api/rbac/v1"` (if not already present), then append:
...
```bash
cd /home/willamhou/codes/k8s4claw && KUBEBUILDER_ASSETS="$(go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use 1.32.x -p path 2>/dev/null)" go test -race -run TestE2E_SelfConfig -v -timeout 120s ./internal/controller/
```
...
```bash
git add internal/controller/e2e_lifecycle_test.go
git commit -m "test: add E2E self-config lifecycle test"
```

---

## Task 8: Run All E2E Tests + Final Verification

**Step 1: Run all E2E tests together**

```bash
cd /home/willamhou/codes/k8s4claw && KUBEBUILDER_ASSETS="$(go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use 1.32.x -p path 2>/dev/null)" go test -race -run 'TestE2E_' -v -timeout 300s ./internal/controller/
```
...
```bash
cd /home/willamhou/codes/k8s4claw && KUBEBUILDER_ASSETS="$(go run sigs.k8s.io/controller-runtime/tools/setup-envtest@latest use 1.32.x -p path 2>/dev/null)" go test -race -timeout 300s ./internal/...
```
...
```bash
cd /home/willamhou/codes/k8s4claw && make build && make vet
```
